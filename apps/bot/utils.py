"""Модуль utils.py для работы с excel файлами с google drive."""
...
def get_credentials() -> Any:
    ...
    credentials = Credentials.from_service_account_file(service_account_file, scopes=_SCOPES)
    return build('drive', 'v3', credentials=credentials)

def get_filenames(drive_service: Any) -> List[Dict[str, str]]:
    results = (
        drive_service.files()
        .list(q=f"'{_FOLDER_ID}' in parents", fields='nextPageToken, files(id, name)')
        .execute()
    )
...
def process_excel2(file_content: BytesIO, teacher_name: str) -> List[Tuple[str, str | int, str, str, bool]]:
    wb = openpyxl.load_workbook(file_content, data_only=True)
    results = []
    ...
    for i, line in enumerate(schedule_text):
        ...
        if line.strip() and line[0].isdigit():
            ...
            if class_hour_day and pair_number in _TIME_MAPPING:
                schedule_text_str = '🕒 ' + schedule_text[i] + f' {_EXTENDED_TIME_MAPPING[pair_number]}'
...
def fetch_file_data(name: str) -> Dict[str, Any]:
    if name in _cache:
        return _cache[name]
    ...
    chosen_file = next((file for file in files if file['name'] == name + '.xlsx'), None)
    if chosen_file is None:
        return {'error': 'Файл не найден.'}

    file_content = download_file(chosen_file['id'], drive_service)
    _cache[name] = {'file_content': file_content}
    return _cache[name]

def service(name: str, group: str) -> str:
    file_data = fetch_file_data(name)
    if 'error' in file_data:
        return file_data['error']

    group_name = group.upper()
    results = process_excel(file_data['file_content'], group_name)
    if not results:
    ...
    for sheet_title, room_number_value, teacher_name, _class_hour in results:
        room_number = int(room_number_value) if isinstance(room_number_value, float) else room_number_value
...
def search_schedule_by_teacher(name: str, teacher_name: str) -> str:
    ...
    results = process_excel2(file_data['file_content'], teacher_name)
    if not results:
